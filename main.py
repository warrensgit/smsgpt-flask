    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": incoming_message  # Using the content from the 'incoming_message' parameter
                }
            ],
            max_tokens=200
        )
...
def delivery_report():
    try:
        ...
        # Basic error handling for missing parameters
        if not all([from_number, to_number, success_code, smsc_status, reference_number, timestamp]):
            return jsonify({"error": "Missing parameters"}), 400

        # Log the delivery report data
        logging.info(f'Delivery report received - From: {from_number}, To: {to_number}, '
...
def process_incoming_message():
    ...
    sms_url = 'https://qa-sms.umsg.co.za/xml/send/?'
    payload = {
        'number': sender,  # Assuming you want to send the response back to the sender
    ...
    if response.status_code == 200:
        ...
        return jsonify({"success": False, "error": "Failed to send SMS"}), response.status_code
